...
;;  sed 'n;d'
(test-sad "undo double-spacing (assumes even-numbered lines are always blank)"
          "printf 'a\n\nb\n\nc\n' | sed 'n;d'"
          ;; sad counts lines from 0
          "printf 'a\n\nb\n\nc\n' | sad eval '(when (even? LINENO) (print INPUT))'")

;;  # insert a blank line above every line which matches "regex"
;;  sed '/regex/{x;p;x;}'
(test-sad "insert a blank line above every line which matches 'regex'"
          "(echo foo; echo bar; echo foo) | sed '/foo/{x;p;x;}'"
          "(echo foo; echo bar; echo foo) | sad eval --match foo '(print \"\n\" INPUT)'")

;;  # insert a blank line below every line which matches "regex"
;;  sed '/regex/G'
(test-sad "insert a blank line below every line which matches 'regex'"
          "(echo foo; echo bar; echo foo) | sed '/foo/G'"
          "(echo foo; echo bar; echo foo) | sad eval --match foo '(print INPUT \"\n\")'")

;;  # insert a blank line above and below every line which matches "regex"
;;  sed '/regex/{x;p;x;G;}'
(test-sad "insert a blank line below every line which matches 'regex'"
          "(echo foo; echo bar; echo foo) | sed '/foo/{x;p;x;G;}'"
          "(echo foo; echo bar; echo foo) | sad eval --match foo '(print \"\n\" INPUT \"\n\")'")
...
;; NUMBERING:
(test-begin "NUMBERING")

;;  # number each line of a file (simple left alignment). Using a tab (see
;;  # note on '\t' at end of file) instead of space will preserve margins.
;;  sed = filename | sed 'N;s/\n/\t/'
(test-sad "number each line of a file (simple left alignment)"
          "seq 3 | sed = | sed 'N;s/\\n/\\t/'"
...
(test-sad "number each line of a file (number on left, right-aligned)"
          "seq 3 15 | sed = | sed 'N; s/^/     /; s/ *\\(.\\{6,\\}\\)\\n/\\1  /'"
          "seq 3 15 | sad eval -R srfi-13 '(print (string-pad (number->string (add1 LINENO)) 6) \"  \" INPUT)'")
...
(test-sad "number each line of file, but only print numbers if line is not blank"
          "seq 3 | awk 'BEGIN{ORS=\"\\n\\n\"};1' | sed '/./=' | sed '/./N; s/\\n/ /'"
          "seq 3 | awk 'BEGIN{ORS=\"\\n\\n\"};1' | \
           sad eval --not-match ^$ '(print (add1 LINENO) \" \" INPUT)'")
